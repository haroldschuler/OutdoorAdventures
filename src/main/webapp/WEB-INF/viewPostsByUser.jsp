...
<head>
	...
	<style>
        body {
            background-image: url("../../images/adventure_posts_by_user.jpg");
...
<body>
    ...
    <div class="content flex-column align-items-center">
        <div class="search_results_title_bar transparent_box">
           	<c:set var="currentUser" scope="session" value="${userId}"/>
           	<c:set var="poster" scope="session" value="${user.id}"/>
            <c:choose>
				<c:when test="${currentUser==poster}">
		            <h1>Here are your posts, ${user.userName}</h1>
				</c:when>
				<c:otherwise>
